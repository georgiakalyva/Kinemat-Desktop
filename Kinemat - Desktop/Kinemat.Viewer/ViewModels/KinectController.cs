...
namespace Kinemat.Viewer.ViewModels
{
	...
	public class KinectController : ViewModelBase
	{
		...
		private readonly TimeSpan disengagementNoHandoffNavigationTimeout = TimeSpan.FromSeconds(30.0);
		...
		/// </summary>
		private bool isPlayerEngagementCandidate;
		...
		/// <summary>
		/// The default navigation screen.
		/// When starting the Kinemat Viewer for the first time we see the loading screen.
		/// </summary>
		private const string welcomeNavigationScreen = NavigableContexts.LoadingScreen;

		/// <summary>
		/// The game navigation screen.
		/// </summary>
		private const string gameNavigationScreen = NavigableContexts.BookScreen;
		...
		public bool IsUserEngaged
		{
			get
			{
				return this.isPlayerEngaged;
			}

			protected set
			{
				bool wasEngaged = this.isPlayerEngaged;

				this.isPlayerEngaged = value;
				this.OnPropertyChanged("IsUserEngaged");

				if (wasEngaged != this.isPlayerEngaged)
				{
		...
		public bool IsUserEngagementCandidate
		{
			get
			{
				return this.isPlayerEngagementCandidate;
			}

			protected set
			{
				this.isPlayerEngagementCandidate = value;
		...
		public bool IsUserTracked
		{
			get
			{
				return this.isPlayerTracked;
			}

			protected set
			{
				this.isPlayerTracked = value;
		...
		private void SensorChooserOnKinectChanged(object sender, KinectChangedEventArgs e)
		{
			...
			this.NavigationManager.NavigateToHome(welcomeNavigationScreen);
		...
		private void OnSkeletonFrameReady(object sender, SkeletonFrameReadyEventArgs e)
		{
			...
			// Do not search for player when loading the game
			if (NavigationManager.CurrentNavigationContext.GetType() == typeof(LoadingScreenViewModel))
				return;
		...
		private void UpdateStartBannerState()
		{
			...
			//PromptState state = PromptState.Prompting;

			if (this.IsUserTracked)
			{
				state = this.IsUserEngagementCandidate || this.IsUserEngaged ? PromptState.Dismissed : PromptState.Prompting;
				// state = this.IsUserEngagementCandidate || this.IsUserEngaged ? PromptState.Dismissed : PromptState.Hidden;
			}
		...
		private void PerformEngagementChangeNavigation()
		{
			...
			else if (!this.disengagementNavigationTimer.IsEnabled)
			{
				if (this.IsUserEngaged)
				{
					...
					this.NavigationManager.NavigateToHome(gameNavigationScreen);
				}
				else
				{
		...
		private void OnNavigationManagerPropertyChanged(object sender, PropertyChangedEventArgs e)
		{
			if ("CurrentNavigationContext".Equals(e.PropertyName))
			{
				...
				{
					isLoading = false;
					this.engagementStateManager.SearchForPlayer = true;
				}					
